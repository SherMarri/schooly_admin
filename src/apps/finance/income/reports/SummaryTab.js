...
const daily_income_options = (data_points) => {
...
class SummaryTab extends React.Component {
    ...
    getCategoryWiseYearlyOptions = () => {
        ...
        const title = `Income Distribution - ${Format(new Date(), 'yyyy')}`;
    ...
    getCategoryWiseMonthlyOptions = () => {
        ...
        const title = `Income Distribution - ${Format(new Date(), 'MMMM yyyy')}`;
        const data_points = category_wise_data.map(d => ({
    ...
    getQuickFacts = () => {
        ...
        let facts = []
        facts.push(`Generated Rs.${Utils.numberWithCommas(summary.yearly_total)} in ${Format(new Date(), 'yyyy')}.`)
        facts.push(`Generated Rs.${Utils.numberWithCommas(summary.monthly_total)} in ${Format(new Date(), 'MMMM, yyyy')}.`)
        facts.push(`Average income is Rs.${Utils.numberWithCommas(Math.ceil(summary.average_item))}.`)

        const most_profitable_category = _.maxBy(summary.category_wise_data, d => d.yearly_total);
        const most_popular_category = _.maxBy(summary.category_wise_data, d => d.item_count);

        facts.push(`${most_profitable_category.name} is the most profitable category.`);
        facts.push(`Most of the income entries belong to ${most_popular_category.name} category.`);
        ...
        return (
            <Grid container spacing={24}>
                ...
                <Grid item xs={12} md={6}>
                    <Paper className={classes.paper}><Chart options={daily_income_options(summary.daily_total)} /></Paper>
                    <Paper className={classes.paper}>{this.renderCategoryIncomeTable()}</Paper>                    
...
function mapStateToProps({ finance }) {
    return {
        summary: finance.income.common.summary,
    }
}
